...
### Security
- **Authorized targets are now a runtime allow-list the agent cannot extend.** Every gated tool call (http_request, send_request, terminal_execute, python_action, browser_action, page_agent, report_vulnerability, authz_matrix, probe_hypothesis) is checked against the scan's configured targets: the target hosts, their resolved addresses, and subdomains of each target's registrable domain (with multi-label public-suffix handling so a `bbc.co.uk` target never authorizes `*.co.uk`). A narrow third-party service set — passive-recon sources, DNS resolvers, package/OS distribution hosts — remains reachable because the engine's recon scripts and auto-install depend on it. Any other discovered host is rejected with guidance to record it as a finding; the runtime now enforces what the system prompt's TARGET SCOPE section always promised. (VDP disclosure, 2026-09-24: an authorized cloud scan read a third-party Supabase backend out of the target's SPA bundle, treated it as "same scope", pivoted to it, and issued writes against production data.)
- **Database/schema reset primitives are hard-blocked on every host** — DROP DATABASE/TABLE/SCHEMA, TRUNCATE, `db:reset`, `migrate:reset`, `prisma migrate reset`, rails/rake/wp database resets, factory resets, Redis `flushdb`/`flushall`, schema wipes — across every traffic-executing tool, authorized targets included. The same disclosure showed an agent reaching for the target's "Create / Reset Database" function as a "repair" for a change it could not revert, destroying the data.
- **State-changing proofs now carry a reversibility contract.** The methodology requires capturing the inverse operation before mutating, restoring immediately after the evidence is captured, and reporting UNRESTORED when restoration fails instead of escalating to a destructive repair.
