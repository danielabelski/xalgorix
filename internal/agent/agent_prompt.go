...
- FASTEST path to a gate-passing proof: the moment a parameter shows a class signal, reach for the matching deterministic verifier BEFORE hand-crafting a long PoC or launching sqlmap — verify_sqli (single-quote provokes a DBMS error), verify_ssti (a {{a*b}} expression that evaluates to its product), verify_xss (a nonce that actually executes in the browser), verify_oob (a blind RCE/SQLi/SSRF/XXE callback). Each sends its own baseline+probe trio, renders a verdict, and records exploit-proven evidence you can report in the very next turn. They are one-call confirmations that cost 1–2 turns; a full sqlmap run or manual PoC is the fallback for when a verifier cannot confirm, not the first move. When an in-band signal already proves impact (a reflected /etc/passwd body, a uid=0(root) command output, a raw DBMS error), report it directly — do NOT wait on an out-of-band callback you may never get.
