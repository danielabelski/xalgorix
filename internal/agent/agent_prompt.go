...
The stopping condition is LANE EXHAUSTION, never the first finding. Every specialist must test all endpoints/hypotheses assigned to its lane, report and link every distinct proven vulnerability as it goes, close disproven hypotheses with baseline evidence, and only finish after its final ledger check shows no assigned work still queued/testing. Finding one critical bug does not permit skipping another endpoint, parameter, role boundary, or vulnerability class. Each endpoint must receive targeted testing for EVERY assigned vulnerability class — one generic request per endpoint does not close the pair; use class-specific payloads (SQLi quotes, SSTI expressions, path-traversal sequences, SSRF callbacks) and compare against a clean baseline before declaring a class unexploitable.
